...
status_t explore_fonts(bela::MemView mv, particulars_result &pr) {
  ...
  case 0x00:
    if (mv.size() > 4 && mv[1] == 0x01 && mv[2] == 0x00 && mv[3] == 0x00 && mv[4] == 0x00) {
      ...
      pr.assign(L"TrueType Font", types::ttf);
      return Found;
    }
    break;
  case 0x4F:
    if (mv.size() > 4 && mv[1] == 0x54 && mv[2] == 0x54 && mv[3] == 0x4F && mv[4] == 0x00) {
      ...
      pr.assign(L"OpenType Font", types::otf);
      return Found;
    }
    break;
  case 0x77:
    ...
    if (mv[1] == 0x4F && mv[2] == 0x46 && mv[3] == 0x46 && mv[4] == 0x00 && mv[5] == 0x01 && mv[6] == 0x00 &&
      ...
      pr.assign(L"Web Open Font Format", types::woff);
      return Found;
    }
    if (mv[1] == 0x4F && mv[2] == 0x46 && mv[3] == 0x32 && mv[4] == 0x00 && mv[5] == 0x01 && mv[6] == 0x00 &&
        mv[7] == 0x00) {
      // https://en.wikipedia.org/wiki/Web_Open_Font_Format
      pr.assign(L"Web Open Font Format 2.0", types::woff2);
      return Found;
    }
    break;
  default:
  ...
  if (IsEot(mv)) {
    pr.assign(L"Embedded OpenType (EOT) fonts", types::eot);
    return Found;
  }
  return None;
}
