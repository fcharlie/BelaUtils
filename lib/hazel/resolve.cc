...
std::wstring guidencode(const GUID &guid) {
  wchar_t wbuf[64];
  swprintf_s(wbuf, L"{%08X-%04X-%04X-%02X%02X%02X%02X%02X%02X%02X%02X}", guid.Data1, guid.Data2, guid.Data3,
...
public:
  ...
  FsRedirection() {
    ...
    auto pfnWow64DisableWow64FsRedirection =
        (fntype_Wow64DisableWow64FsRedirection *)GetProcAddress(hModule, "Wow64DisableWow64FsRedirection");
  ...
  ~FsRedirection() {
    ...
    auto pfnWow64RevertWow64FsRedirection =
        (fntype_Wow64RevertWow64FsRedirection *)GetProcAddress(hModule, "Wow64RevertWow64FsRedirection");
...
std::optional<file_target_t> ResolveTarget(std::wstring_view sv, bela::error_code &ec) {
  ...
  FsRedirection fsr;
#endif
  auto FileHandle = CreateFileW(sv.data(), 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, nullptr,
  ...
  DWORD dwlen = 0;
  if (DeviceIoControl(FileHandle, FSCTL_GET_REPARSE_POINT, nullptr, 0, rebuf, MAXIMUM_REPARSE_DATA_BUFFER_SIZE, &dwlen,
                      nullptr) != TRUE) {
