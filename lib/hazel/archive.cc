...
status_t inquisitive_7zinternal(bela::MemView mv, particulars_result &pr) {
  constexpr const byte_t k7zSignature[k7zSignatureSize] = {'7', 'z', 0xBC, 0xAF, 0x27, 0x1C};
  constexpr const byte_t k7zFinishSignature[k7zSignatureSize] = {'7', 'z', 0xBC, 0xAF, 0x27, 0x1C + 1};
  ...
  pr.assign(std::move(buf), types::p7z);
  return Found;
}

// RAR archive
// https://www.rarlab.com/technote.htm
status_t inquisitive_rarinternal(bela::MemView mv, particulars_result &pr) {
  ...
  if (mv.StartsWith(rarSignature)) {
    pr.assign(L"Roshal Archive (rar), version 5", types::rar);
    return Found;
  }
  if (mv.StartsWith(rar4Signature)) {
    pr.assign(L"Roshal Archive (rar), version 4", types::rar);
    return Found;
  }
  return None;
}
...
status_t inquisitive_xarinternal(bela::MemView mv, particulars_result &pr) {
  ...
  pr.assign(std::move(name), types::xar);
  return Found;
}
...
status_t inquisitive_dmginternal(bela::MemView mv, particulars_result &pr) {
  ...
  pr.assign(std::move(name), types::dmg);
  return Found;
}

// PDF file format
status_t inquisitive_pdfinternal(bela::MemView mv, particulars_result &pr) {
  ...
  pr.assign(std::move(buf), types::pdf);
  return Found;
}
...
status_t inquisitive_wiminternal(bela::MemView mv, particulars_result &pr) {
  ...
  pr.assign(std::move(name), types::wim);
  if ((flag & WimCompression) != 0) {
    ...
    pr.add(L"Compression", std::move(compression));
  }

  pr.add(L"Imagecount", bela::AlphaNum(bela::swaple(hd->dwImageCount)).Piece());
  pr.add(L"TotalParts", bela::AlphaNum(bela::swaple(hd->usTotalParts)).Piece());
  pr.add(L"PartNumber", bela::AlphaNum(bela::swaple(hd->usPartNumber)).Piece());
...
status_t inquisitive_cabinetinternal(bela::MemView mv, particulars_result &pr) {
  ...
  pr.assign(std::move(name), types::cab);
  return Found;
}
...
status_t inquisitive_tarinternal(bela::MemView mv, particulars_result &pr) {
  ...
  if (memcmp(hd->magic, ustarMagic, ArrayLength(ustarMagic)) == 0) {
    pr.assign(L"Tarball (ustar) archive data", types::tar);
    return Found;
  }
  if (memcmp(hd->magic, gnutarMagic, ArrayLength(gnutarMagic)) == 0 && mv.size() > sizeof(gnutar_header_t)) {
    pr.assign(L"Tarball (gnutar) archive data", types::tar);
    return Found;
  }
  return None;
}
...
status_t inquisitive_sqliteinternal(bela::MemView mv, particulars_result &pr) {
  constexpr const byte_t sqliteMagic[] = {'S', 'Q', 'L', 'i', 't', 'e', ' ', 'f', 'o', 'r', 'm', 'a', 't'};
  ...
  pr.assign(std::move(name), types::sqlite);
  return Found;
}
...
status_t inquisitive_archivesinternal(bela::MemView mv, particulars_result &pr) {
  ...
  if (mv.StartsWith(bz2Magic)) {
    pr.assign(L"BZ2 archive data", types::bz2);
    return Found;
  }
  ...
  if (mv.StartsWith(nesMagic)) {
    pr.assign(L"Nintendo NES ROM", types::nes);
    return Found;
  }
  ...
  if (mv.StartsWith(zMagic)) {
    pr.assign(L"X compressed archive data", types::z);
    return Found;
  }

  constexpr const byte_t lzMagic[] = {0x4C, 0x5A, 0x49, 0x50};
  if (mv.StartsWith(lzMagic)) {
    pr.assign(L"LZ archive data", types::lz);
    return Found;
  }
  // SWF
  if (IsSwf(mv.data(), mv.size())) {
    pr.assign(L"Adobe Flash file format", types::swf);
    return Found;
  }
  // EPUB
  if (IsEPUB(mv.data(), mv.size())) {
    pr.assign(L"EPUB document", types::epub);
    return Found;
  }
  return None;
}
...
status_t explore_archives(bela::MemView mv, particulars_result &pr) {
  if (inquisitive_7zinternal(mv, pr) == Found) {
    return Found;
  }
  if (inquisitive_rarinternal(mv, pr) == Found) {
    return Found;
  }
  if (inquisitive_xarinternal(mv, pr) == Found) {
    return Found;
  }
  if (inquisitive_dmginternal(mv, pr) == Found) {
    return Found;
  }
  if (inquisitive_pdfinternal(mv, pr) == Found) {
    return Found;
  }
  if (inquisitive_wiminternal(mv, pr) == Found) {
    return Found;
  }
  if (inquisitive_cabinetinternal(mv, pr) == Found) {
    return Found;
  }
  if (inquisitive_tarinternal(mv, pr) == Found) {
    return Found;
  }
  if (inquisitive_sqliteinternal(mv, pr) == Found) {
    return Found;
  }
  return inquisitive_archivesinternal(mv, pr);
}
} // namespace hazel::internal
