...
status_t explore_git_file(bela::MemView mv, particulars_result &pr) {
  ...
  if (mv.StartsWith(packMagic)) {
    ...
    pr.assign(std::move(name), types::gitpack);
    return Found;
  }
  if (mv.StartsWith(indexMagic)) {
    ...
    pr.assign(std::move(name), types::gitpkindex);
    return Found;
  }
  if (mv.StartsWith(midxMagic)) {
    ...
    auto name =
        bela::StringCat(L"Git multi-pack-index, version ", (int)hd->version, L", oid version ", (int)hd->oidversion,
                        L", chunks ", (int)hd->chunks, L", pack files ", bela::swapbe(hd->packfiles));
    pr.assign(std::move(name), types::gitpack);
    return Found;
  }
