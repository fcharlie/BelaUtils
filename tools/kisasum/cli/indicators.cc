...
void ProgressBar::Draw() {
  ...
  if (maximum == 0) {
    ...
    bela::FPrintF(stderr, L"\x1b[2K\r\x1b[01;%dm[%s%s%s] %s %s/s\x1b[0m", (uint32_t)state, s0,
  ...
  bela::FPrintF(stderr, L"\x1b[2K\r\x1b[01;%dm%d%% [%s%s] %s %s/s\x1b[0m", (uint32_t)state, scale,
